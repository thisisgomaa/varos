...
>
> **🟡 NEXT milestone — masks/clipping** (spec'd §3 of `LAYERS_VISION.md`, NOT started): `paint_list()` insurance FIRST → additive `Node` fields + `sync_tree` guards → `build_scene`→`emit_node` tree-walk refactor → `Clip` variant + stencil pipelines + the two gestures (drag-onto-thumbnail + Ctrl+Alt+G). Structural bands = deferred (Ahmed: "drop for now"; spec kept, zero-rework to switch on).
>
> **🐞 Known issues (from the 2026-07-04 planning review — hand these to the Layers session before masks, since masks build on this panel):**
> 1. **`layer_toggle` — Ctrl+click can't deselect a row that has a locked/hidden member** (`editor.rs:1917`). `all_in` requires every path in `objsel`, but locked/hidden paths are never inserted there, so `all_in` is never true → Ctrl+click only ever re-adds, never removes, for mixed-lock rows. Fix: compute `all_in` over the *insertable* (unlocked, unhidden) paths only. No test covers `layer_toggle`.
> 2. **Dead code after the decoration cleanup** — 6 orphaned `pub fn` in `editor.rs` (`layer_new`, `layer_new_sublayer`, `layer_delete`, `layer_move_art`, `layer_focus`, `layer_select_art`) + `model.rs` `add_layer`/`add_sublayer` reachable only through them. Zero call sites; `dead_code` lint stays silent because they're `pub` on a lib crate, so "compiles clean" hid it. The commit said "model fields stay, just not drawn" but it also left live methods. Fix: delete them (or `#[allow(dead_code)]` + a note if a near-future use is known).
