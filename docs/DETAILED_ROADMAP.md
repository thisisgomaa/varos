...
>
> **🟡 NEXT milestone — masks/clipping** (spec'd §3 of `LAYERS_VISION.md`): **① `paint_list()` insurance ✅ 2026-07-06** (`PaintRole` + index-preserving `paint_list()` in the model; scene content loop, PDF export and snap targets read it; golden test proves byte-identity with `doc.paths` today) → ② additive `Node` fields + `sync_tree` guards → ③ `build_scene`→`emit_node` tree-walk refactor → ④ `Clip` variant + stencil pipelines → ⑤ the two gestures (drag-onto-thumbnail + Ctrl+Alt+G). Structural bands = deferred (Ahmed: "drop for now"; spec kept, zero-rework to switch on).
>
