...
- **Next queued pieces:** Ahmed's D1–D3 on `docs/specs/DOCUMENT_FILE_SYSTEM.md` (unblocks S1: real tabs / New / lifecycle — closes F02, and later S6 Export — closes F03); Codex review + hand test of Astra batch 1, then merge to `main`; Astra F08 (thick-stroke hit-testing), F10 (inline rename), F11 (small-control readability); P11.2 item (0), instant zoom — **DONE** `36d04d4`; ⌘ labels on Mac — **DONE** `36d04d4`; icon library S1 (`docs/studies/2026-09-23-ICON_LIBRARY_STUDY.md`).
...
| Workspace tests | 306 — whole workspace, run on macOS 2026-09-24 on merged `main` at `36d04d4` (0 failed) · branch `claude/sweet-cerf-1sg30t` at `0e369a6`: 346 on Linux (cloud; Mac-only tests not compiled there) |
