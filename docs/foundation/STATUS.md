...
**Decisions waiting for Ahmed (each with the recommended default):** accept ADR-0008 (format v2) [yes]; UI system: adopt as one priority after DFS S1 [yes, after the spec v2]; bundle IBM Plex Sans/Mono + Plex Sans Arabic [yes; multi-word Arabic names need a bidi spike]; remove the drag ease + glide + azure glow, keep the direction bar [yes, with ADR-0009 for the glide]; the mockup's "on" looks (tool = azure block, toggle = small azure bar, tab = surface fill) [yes]; labels FAINT → MUTED with QW6 sizes [yes]; layout persistence deferred, Reset Workspace in the Window menu only [yes]; broken clip mask in an old file: open with the mask released + notice [yes]; export name when the document is itself a `.pdf`: `<name> export.pdf` [yes].
...
- **Next queued pieces:** Codex review + hand test of Astra batch 1 and of the afternoon wave (S3-A, S1-A, QW3, QW1, S6-A), then merge `claude/sweet-cerf-1sg30t` to `main`; Ahmed's acceptance of `docs/adr/ADR-0008-vrs-format-versioning.md` (currently Proposed) to unblock S5 format-v2 code; S1-B/C/D, S3-B/C, S5-B/E and QW4 (round caps)/QW5 (Edit-menu rows) in flight; Astra F11 (small-control readability); UI system initiative under way — 5 audits landed in `docs/audits/ui-2026-09-24/`, defect backlog/glue map to be worked next; icon library S1 (`docs/studies/2026-09-23-ICON_LIBRARY_STUDY.md`).
...
| Workspace tests | 306 — whole workspace, run on macOS 2026-09-24 on merged `main` at `36d04d4` (0 failed) · branch `claude/sweet-cerf-1sg30t` at `0e369a6`: 346 on Linux (cloud; Mac-only tests not compiled there) · branch `claude/sweet-cerf-1sg30t` at `9bd3389` (afternoon wave, 2026-09-24): **459/459** on Linux (cloud; Mac-only tests not compiled there) · branch head `6999970`: 624/624 (Linux, 4 ignored by design) |
| Tests on macOS | 306 / 306 — whole workspace incl. `varos-app` (2026-09-24, merged `main` at `36d04d4`) |
