...
- **Next queued pieces:** Astra (Codex computer-use) user test — BLOCKED, see below; P11.2 item (0), instant zoom — **DONE** `36d04d4`; ⌘ labels on Mac — **DONE** `36d04d4`; icon library S1 (`docs/studies/2026-09-23-ICON_LIBRARY_STUDY.md`).
...
- **Astra hand test BLOCKED (2026-09-24):** macOS app-control permission for Varos blocked Codex computer-use. The owner must allow control of Varos in the Codex app, then retry the hand test. Brief staged at `varos/target/astra-brief.txt`; batch 1 remains pending.
...
**Known Mac gaps (expected, not bugs of this piece):** screen eyedropper disabled; window position/size not remembered; Quit/Close do not prompt for unsaved changes (same as ✕ today); menu items without an existing shortcut are omitted (New, Export, Cut/Copy/Paste, Duplicate, Select All, Deselect, Zoom In/Out, Delete).
...
| Workspace tests | 306 — whole workspace, run on macOS 2026-09-24 on merged `main` at `36d04d4` (0 failed) |
