...
is Windows-bound. This piece makes it build and launch on macOS with **zero change to any Windows
code path**.
...
2. egui texture uploads ran *after* acquiring the frame; when the OS gave no frame (occluded /
   ...
   ("texture that has not been allocated yet"). Fix: upload textures before acquiring the frame.
