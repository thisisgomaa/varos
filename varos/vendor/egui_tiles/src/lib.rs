...
impl DropContext {
    fn on_tile<Pane>(
        ...
        if !rect.contains(mouse) || self.best_dist_sq <= 0.0 {
            return; // outside this box, or an outer box already claimed the cursor this pass
        }
        if let Tile::Pane(pane) = tile
            && !behavior.pane_is_drop_target(pane)
        {
