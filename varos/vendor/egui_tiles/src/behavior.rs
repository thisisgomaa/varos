...
pub trait Behavior<Pane> {
    ...
    /// Can another tile be DROPPED ONTO this pane (tabbed / docked into it)?
    ///
    /// LOCAL FORK (Varos): the canvas/board returns `false` so panels never tab or dock *into* it.
    /// Default: `true`.
    fn pane_is_drop_target(&self, _pane: &Pane) -> bool {
        true
    }
