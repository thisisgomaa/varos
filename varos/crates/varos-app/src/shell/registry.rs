...
use egui::{Align2, FontId, RichText, Sense, StrokeKind, Vec2, vec2};
...
impl PanelId {
    ...
    pub const DOCKABLE: [PanelId; 7] = [
        PanelId::Align, PanelId::Pathfinder, PanelId::Properties,
        PanelId::Layers, PanelId::Swatches, PanelId::History, PanelId::Assets,
    ];
    ...
    pub fn is_board(self) -> bool { matches!(self, PanelId::Board) }

    /// Minimum content size hint (points). Used by the tree for min-size clamps (BOX_SYSTEM_PLAN §3.5).
    pub fn min_size(self) -> Vec2 {
        if self.is_board() { vec2(240.0, 180.0) } else { vec2(190.0, 120.0) }
    }
}
...
fn icon_btn(ui: &mut egui::Ui, glyph: &str, on: bool) {
    ...
    let p = ui.painter();
    if on { p.rect_filled(rect, T::r_ctrl(), T::HOVER); }
    else if resp.hovered() { p.rect_filled(rect, T::r_ctrl(), T::VOID_HOVER); }
    let col = if on { T::TEXT } else if resp.hovered() { T::TEXT } else { T::MUTED };
...
fn r2() -> egui::CornerRadius { egui::CornerRadius::same(2) }

// ───────────────────────── the dummy panels ─────────────────────────

fn align_panel(ui: &mut egui::Ui) {
    micro(ui, "ALIGN OBJECTS");
    ui.horizontal(|ui| { for g in ["⇤", "⇔", "⇥", "⤒", "⇕", "⤓"] { icon_btn(ui, g, false); ui.add_space(2.0); } });
    ui.add_space(8.0);
    micro(ui, "DISTRIBUTE");
    ui.horizontal(|ui| { for g in ["≡", "≣", "⋮", "⋯"] { icon_btn(ui, g, false); ui.add_space(2.0); } });
    ui.add_space(10.0);
...
fn pathfinder_panel(ui: &mut egui::Ui) {
    ...
    ui.horizontal(|ui| { for g in ["◕", "◑", "◔", "◒"] { icon_btn(ui, g, false); ui.add_space(3.0); } });
    ui.add_space(8.0);
    micro(ui, "PATHFINDERS");
    ui.horizontal(|ui| { for g in ["⬒", "⬓", "⬔", "⬕"] { icon_btn(ui, g, false); ui.add_space(3.0); } });
    ui.add_space(10.0);
    ui.horizontal(|ui| { icon_btn(ui, "⊙", true); ui.label(RichText::new("Shape Builder").color(T::MUTED).size(11.5)); });
}
...
fn layers_panel(ui: &mut egui::Ui) {
    for (name, depth) in [("Logo mark", 0), ("Circle", 1), ("Star", 1), ("Text · VAROS", 0), ("Background", 0)] {
        ...
        let p = ui.painter();
        if resp.hovered() { p.rect_filled(rect, T::r_ctrl(), T::VOID_HOVER); }
        ...
        p.rect(egui::Rect::from_min_size(egui::pos2(x0 + 18.0, rect.center().y - 8.0), vec2(16.0, 16.0)), r2(), T::SURFACE, T::hairline(), StrokeKind::Middle); // thumb
...
fn swatches_panel(ui: &mut egui::Ui) {
    ...
    ui.horizontal_wrapped(|ui| { for c in cols { swatch(ui, c); ui.add_space(4.0); } });
}
...
fn history_panel(ui: &mut egui::Ui) {
    ...
    for (i, step) in ["Open document", "Draw ellipse", "Fill amber", "Add text", "Move ×3"].iter().enumerate() {
        ...
        let on = i == 3;
        let p = ui.painter();
        if on { p.rect_filled(rect, T::r_ctrl(), T::SURFACE); } else if resp.hovered() { p.rect_filled(rect, T::r_ctrl(), T::VOID_HOVER); }
        p.text(egui::pos2(rect.left() + 10.0, rect.center().y), Align2::LEFT_CENTER, *step, FontId::proportional(11.5), if on { T::TEXT } else { T::MUTED });
    }
}
