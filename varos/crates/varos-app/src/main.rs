...
/// Which native cursor the current effective tool wants (Pen reports its contextual state).
fn desired_ck(ed: &Editor, world: Pt) -> CK {
    match ed.eff_tool() {
        ToolKind::Object => CK::Select,
...
fn main() {
    ...
    // native cursors: subclass the canvas window + build the Lucide HCURSOR set
    let hwnd: isize = {
    ...
    cursors::install(hwnd);
    let hcur: std::collections::HashMap<CK, isize> = cursors::ALL.iter().map(|&ck| (ck, cursors::hcursor(ck))).collect();
    cursors::set(hcur[&CK::Pen]); // initial (Pen is the default tool)
    ...
    event_loop.run(move |event, elwt| {
        ...
        Event::UserEvent(UserEvent::Ipc(msg)) => {
            if msg == "ready" {
                // a panel loaded → give it the matching custom cursor (so it isn't the Windows arrow)
                let js = format!("document.documentElement.style.cursor={0:?};document.body.style.cursor={0:?};", panel_css);
                for w in [&topbar, &tools_panel, &panel, &zoom_panel] { let _ = w.evaluate_script(&js); }
        ...
        Event::WindowEvent { event, window_id } => {
            ...
            match event {
                ...
                WindowEvent::RedrawRequested => {
                    ed.ppu = view.zoom;
                    let ck = desired_ck(&ed, view.s2w(screen_cursor));
                    if Some(ck) != last_ck { cursors::set(hcur[&ck]); last_ck = Some(ck); }
