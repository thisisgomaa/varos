...
fn rect(id: u32, base: u32, fill: Option<Rgba>, stroke: Option<Rgba>) -> Path {
    Path::new(
        id,
        vec![anc(base, 0.0, 0.0), anc(base + 1, 100.0, 0.0), anc(base + 2, 100.0, 100.0), anc(base + 3, 0.0, 100.0)],
        true,
        fill,
        stroke,
        4.0,
...
fn n_isolated(groups: &[Group]) -> usize {
    groups.iter().filter(|g| matches!(g, Group::Isolated { .. })).count()
}
...
fn translucent_fill_plus_stroke_becomes_one_isolated_layer() {
    ...
    let iso = s
        ...
        .iter()
        .find_map(|g| match g {
            Group::Isolated { opacity, prims } => Some((opacity, prims)),
            _ => None,
        })
        .unwrap();
    ...
    let has_full_stroke =
        iso.1.iter().any(|p| matches!(p, Prim::Stroke { color, .. } if (color[3] - 1.0).abs() < 1e-6));
...
fn opaque_objects_paint_fill_then_own_stroke_in_z_order() {
    ...
    let kinds: Vec<char> = s.content[0]
        .prims()
        .iter()
        .map(|p| match p {
            Prim::Fill { .. } => 'F',
...
fn translucent_stroke_on_filled_object_becomes_knockout() {
    ...
    let s = build_scene(&ed, 1.0);
    assert!(
        s.content.iter().any(|g| matches!(g, Group::Knockout(_))),
        "fill + translucent stroke must emit a Knockout group"
    );
...
fn isolated_layer_keeps_its_z_position() {
    ...
    assert!(
        matches!(s.content.last(), Some(Group::Isolated { .. })),
        "the translucent-in-front object is the last group"
    );
