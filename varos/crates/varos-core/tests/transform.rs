...
fn rect(id: u32, ids: [u32; 4], x: f32, y: f32, w: f32, h: f32) -> Path {
    Path::new(id, vec![anc(ids[0], x, y), anc(ids[1], x + w, y), anc(ids[2], x + w, y + h), anc(ids[3], x, y + h)],
...
fn rotate_tool_rotates_around_the_clicked_pivot() {
    let mut ed = sel_rect();
    ed.pointer_down([0.0, 0.0]); ed.pointer_up();     // a plain click relocates the pivot to the corner [0,0]
    ...
    assert!((p2[0]).abs() < 0.5 && (p2[1] - 100.0).abs() < 0.5, "corner [100,0] should rotate to ~[0,100], got {:?}", p2);
    ed.pointer_up();
}
...
fn pivot_click_snaps_to_a_nearby_anchor() {
    ...
    ed.pointer_down([103.0, 3.0]); ed.pointer_up();   // click ~4px from the corner -> should snap onto it
    let piv = ed.pivot.expect("a click sets the pivot");
    assert!((piv[0] - 100.0).abs() < 0.01 && piv[1].abs() < 0.01, "a pivot click near a corner must snap onto ~[100,0], got {:?}", piv);
}
...
fn scale_tool_scales_around_the_pivot() {
    ...
    ed.pointer_down([0.0, 0.0]); ed.pointer_up();     // pivot -> [0,0]
...
fn transform_again_repeats_the_rotate_copy() {
    let mut ed = sel_rect();
    ed.pointer_down([0.0, 0.0]); ed.pointer_up();     // pivot -> corner [0,0]
