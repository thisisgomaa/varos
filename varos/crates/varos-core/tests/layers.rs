...
fn tri(id: u32, base: u32, x: f32) -> Path {
    Path::new(id, vec![anc(base, x, 0.0), anc(base + 1, x + 20.0, 0.0), anc(base + 2, x, 20.0)],
...
fn legacy_registry_files_migrate_with_z_preserved() {
    ...
    let mut d = Document::default();
    d.nodes.clear(); d.roots.clear(); d.active_layer = 0;   // simulate a legacy file exactly
    ...
    let mut m = d.group_members(2); m.sort();
...
fn hiding_or_locking_a_container_cascades() {
    ...
    if let Some(i) = ed.doc.nodes.iter().position(|n| n.id == gid) { ed.doc.nodes[i].hidden = true; }
    ...
    if let Some(i) = ed.doc.nodes.iter().position(|n| n.id == gid) { ed.doc.nodes[i].hidden = false; ed.doc.nodes[i].locked = true; }
