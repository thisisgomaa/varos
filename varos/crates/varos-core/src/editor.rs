...
impl ToolKind {
    pub fn is_shape(self) -> bool {
        matches!(self, ToolKind::Rect | ToolKind::Ellipse | ToolKind::Triangle | ToolKind::Polygon)
    }
...
pub enum TfHit {
    Scale(u8),
    Rotate(u8),
} // u8 = handle index 0..7 (corners 0-3, edge mids 4-7)
...
fn make_anchor(id: u32, p: Pt, hin_raw: Pt, hout_raw: Pt) -> Anchor {
    let din = sub(hin_raw, p);
    let dout = sub(hout_raw, p);
...
fn contour_segs(anchors: &[Anchor]) -> Vec<Seg> {
    ...
    (0..n)
        .map(|i| {
            let x = &anchors[i];
            let y = &anchors[(i + 1) % n];
            (x.p, x.hout.unwrap_or(x.p), y.hin.unwrap_or(y.p), y.p)
        })
        .collect()
}
...
impl Editor {
    ...
    pub fn frame_handles(&self) -> Option<[Pt; 8]> {
        ...
        let th = self.obj_angle;
        let mut o = [[0.0f32; 2]; 8];
        for i in 0..8 {
            o[i] = rotate_about(l[i], [0.0, 0.0], th);
        }
        Some(o)
    }
    ...
    pub fn frame_corners(&self) -> Option<[Pt; 4]> {
        ...
        let th = self.obj_angle;
        Some([
            rotate_about([x0, y0], [0.0, 0.0], th),
            rotate_about([x1, y0], [0.0, 0.0], th),
            rotate_about([x1, y1], [0.0, 0.0], th),
            rotate_about([x0, y1], [0.0, 0.0], th),
        ])
    }
    fn opposite(i: u8) -> u8 {
        match i {
    ...
    fn translate_path(&mut self, pi: usize, d: Pt) {
        ...
        for h in &mut self.doc.paths[pi].holes {
            for a in h {
                a.p = add(a.p, d);
                a.hin = a.hin.map(|x| add(x, d));
                a.hout = a.hout.map(|x| add(x, d));
            }
        }
    }
    ...
    pub fn arrange(&mut self, op: ZOrder) {
        ...
        let (toward_front, extreme) = match op {
            ZOrder::Front => (true, true),
            ZOrder::Back => (false, true),
            ZOrder::Forward => (true, false),
            ZOrder::Backward => (false, false),
        };
    ...
    fn align_anchors(&mut self, mode: AlignMode) {
        ...
        let (mut x0, mut y0, mut x1, mut y1) = (f32::MAX, f32::MAX, f32::MIN, f32::MIN);
        for &id in &ids {
            if let Some(a) = self.doc.anchor(id) {
                x0 = x0.min(a.p[0]);
                y0 = y0.min(a.p[1]);
                x1 = x1.max(a.p[0]);
                y1 = y1.max(a.p[1]);
        ...
        for &id in &ids {
            if let Some(a) = self.doc.anchor_mut(id) {
                let np = match mode {
                    AlignMode::Left => [x0, a.p[1]],
                    AlignMode::Right => [x1, a.p[1]],
                    AlignMode::CenterH => [(x0 + x1) * 0.5, a.p[1]],
                    AlignMode::Top => [a.p[0], y0],
                    AlignMode::Bottom => [a.p[0], y1],
                    AlignMode::Middle => [a.p[0], (y0 + y1) * 0.5],
                };
                let dx = [np[0] - a.p[0], np[1] - a.p[1]];
                a.p = np;
                a.hin = a.hin.map(|h| add(h, dx));
                a.hout = a.hout.map(|h| add(h, dx));
            }
        }
        self.dirty = true;
        self.commit();
    ...
    fn distribute_anchors(&mut self, axis: DistAxis) {
        let mut items: Vec<(u32, f32)> = self
            ...
            .iter()
            .filter_map(|&id| {
                self.doc.anchor(id).map(|a| {
                    (
                        id,
                        match axis {
                            DistAxis::Horizontal => a.p[0],
                            DistAxis::Vertical => a.p[1],
                        },
                    )
                })
            })
        ...
        for k in 1..n - 1 {
            ...
            if let Some(a) = self.doc.anchor_mut(items[k].0) {
                let dx = match axis {
                    DistAxis::Horizontal => [target - a.p[0], 0.0],
                    DistAxis::Vertical => [0.0, target - a.p[1]],
                };
                a.p = add(a.p, dx);
                a.hin = a.hin.map(|h| add(h, dx));
                a.hout = a.hout.map(|h| add(h, dx));
    ...
    pub fn distribute(&mut self, axis: DistAxis) {
        if !self.selected.is_empty() {
            self.distribute_anchors(axis);
            return;
        } // Direct Selection: distribute points
        if self.objsel.len() < 3 {
            return;
        }
        let mut items: Vec<(u32, f32)> = self
            .objsel
            .iter()
            .filter_map(|&pid| {
                self.doc.pidx(pid).map(|pi| {
                    let b = self.doc.outline_bbox(pi);
                    (
                        pid,
                        match axis {
                            DistAxis::Horizontal => (b.0 + b.2) * 0.5,
                            DistAxis::Vertical => (b.1 + b.3) * 0.5,
                        },
                    )
                })
            })
        ...
        for (k, &(pid, c)) in items.iter().enumerate().skip(1).take(n - 2) {
            ...
            if let Some(pi) = self.doc.pidx(pid) {
                self.translate_path(
                    pi,
                    match axis {
                        DistAxis::Horizontal => [d, 0.0],
                        DistAxis::Vertical => [0.0, d],
                    },
                );
            }
        }
    ...
    pub fn distribute_spacing(&mut self, axis: DistAxis, gap: f32) {
        ...
        let mut items: Vec<(u32, f32, f32)> = self
            .objsel
            .iter()
            .filter_map(|&pid| {
                self.doc.pidx(pid).map(|pi| {
                    let b = self.doc.outline_bbox(pi);
                    match axis {
                        DistAxis::Horizontal => (pid, b.0, b.2 - b.0),
                        DistAxis::Vertical => (pid, b.1, b.3 - b.1),
                    }
                })
            })
            .collect();
    ...
    pub fn transform_again(&mut self) {
        let (tf, was_copy) = match self.last_tf {
            Some(t) => t,
            None => return,
        };
        if self.objsel.is_empty() {
    ...
    pub fn ab_hit(&self, pos: Pt) -> Option<AbHit> {
        if let Some(ab) = self.doc.active_artboard() {
            ...
            for (i, h) in Self::ab_handles(ab).iter().enumerate() {
                if dist(pos, *h) <= r {
                    return Some(AbHit::Handle(i as u8));
                }
            }
        }
        for i in (0..self.doc.artboards.len()).rev() {
            if self.doc.artboards[i].contains(pos) {
                return Some(AbHit::Body(i));
            }
        }
        None
    }
    ...
    pub fn ab_orient(&mut self, i: usize) {
        self.begin();
        if let Some(ab) = self.doc.artboards.get_mut(i) {
            std::mem::swap(&mut ab.w, &mut ab.h);
        }
        self.dirty = true;
        self.commit();
    }
    ...
    fn snap_lines_ex(
        ...
        for (pi, p) in self.doc.paint_list() {
            ...
            let b = self.doc.outline_bbox(pi);
            if cfg.object_bounds {
                txl.push((b.0, b.1, b.3));
                txl.push((b.2, b.1, b.3));
                tyl.push((b.1, b.0, b.2));
                tyl.push((b.3, b.0, b.2));
            }
            if cfg.bbox_mids {
                txl.push(((b.0 + b.2) * 0.5, b.1, b.3));
                tyl.push(((b.1 + b.3) * 0.5, b.0, b.2));
            }
        }
        if cfg.artboard {
            let mut push_ab = |ab: &Artboard| {
                ...
                tyl.push((ay0, ax0, ax1));
                tyl.push((ay1, ax0, ax1));
        ...
        if cfg.guides && !self.guides_hidden {
            ...
            for g in &self.doc.guides {
                if g.vertical {
                    txl.push((g.pos, -1.0e6, 1.0e6));
                } else {
                    tyl.push((g.pos, -1.0e6, 1.0e6));
                }
            }
        }
        (txl, tyl)
    ...
    pub fn adaptive_grid_step(&self) -> f32 {
        ...
        if step * zoom < 9.0 {
            ...
            step
        } // too dense to draw → next level up
    }
    ...
    fn equal_gap(&self, axis: usize, m0: f32, m1: f32, p0: f32, p1: f32, tol: f32) -> Option<(f32, Vec<SnapGuide>)> {
        ...
        for &g in &gaps {
            // match an existing gap (extend the row)
            if below > f32::MIN {
                let t = below + g;
                cands.push((t, vec![(below, t)]));
            }
            if above < f32::MAX {
                let t = above - g - len;
                cands.push((t, vec![(t + len, above)]));
            }
        }
        ...
        best.map(|(diff, segs)| {
            ...
            let guides = segs
                ...
                .map(|(a, b)| {
                    if axis == 0 {
                        SnapGuide::Gap { a: [a, pm], b: [b, pm] }
                    } else {
                        SnapGuide::Gap { a: [pm, a], b: [pm, b] }
                    }
                })
                .collect();
            (diff, guides)
    ...
    fn snap_move_lines(
        ...
        let mut by: Option<(f32, f32, f32, f32)> = None;
        ...
        if let Some((diff, x, s0, s1)) = bx {
            nd[0] += diff;
            sx_done = true;
            if cfg.alignment_guides {
                guides.push(SnapGuide::Line { a: [x, s0], b: [x, s1] });
            }
        }
        if let Some((diff, y, s0, s1)) = by {
            nd[1] += diff;
            sy_done = true;
            if cfg.alignment_guides {
                guides.push(SnapGuide::Line { a: [s0, y], b: [s1, y] });
            }
        }
        ...
        if bx_center && by_center {
            if let (Some((_, x, _, _)), Some((_, y, _, _))) = (bx, by) {
                let r = 5.0;
        ...
        if gaps && cfg.gaps_and_sizes && cfg.equal_spacing {
            ...
            if !sy_done {
                if let Some((diff, mut g)) = self.equal_gap(1, my0, my1, mx0, mx1, tol) {
                    nd[1] += diff;
                    sy_done = true;
                    guides.append(&mut g);
                }
    ...
    pub fn snap_origin(&self, p: Pt) -> Pt {
        ...
        if let Some(ab) = self.doc.active_artboard() {
            ...
            xs.extend([x0, x1, (x0 + x1) * 0.5]);
            ys.extend([y0, y1, (y0 + y1) * 0.5]);
        }
        for pi in 0..self.doc.paths.len() {
            ...
            let b = self.doc.outline_bbox(pi);
            xs.extend([b.0, b.2, (b.0 + b.2) * 0.5]);
            ys.extend([b.1, b.3, (b.1 + b.3) * 0.5]);
    ...
    pub fn guide_at(&self, pos: Pt) -> Option<usize> {
        if self.guides_hidden || self.doc.guides_locked {
            return None;
        }
    ...
    fn snap_axis(&self, v: f32, vertical: bool) -> f32 {
        ...
        if let Some(ab) = self.doc.active_artboard() {
            let (x0, y0, x1, y1) = ab.rect();
            if vertical {
                cands.extend([x0, x1, (x0 + x1) * 0.5]);
            } else {
                cands.extend([y0, y1, (y0 + y1) * 0.5]);
            }
        }
        for pi in 0..self.doc.paths.len() {
            let p = &self.doc.paths[pi];
            ...
            let b = self.doc.outline_bbox(pi);
            if vertical {
                cands.extend([b.0, b.2, (b.0 + b.2) * 0.5]);
            } else {
                cands.extend([b.1, b.3, (b.1 + b.3) * 0.5]);
            }
            for a in p.anchors.iter().chain(p.holes.iter().flatten()) {
                cands.push(if vertical { a.p[0] } else { a.p[1] });
            }
        }
    ...
    pub fn commit_guide(&mut self) {
        if let Some(g) = self.guide_preview.take() {
            self.begin();
            self.doc.guides.push(g);
            self.dirty = true;
            self.commit();
        }
    }
    /// Cancel an in-progress ruler drag-out without placing a guide.
    pub fn cancel_guide(&mut self) {
    ...
    fn snap_to_points(&self, base_pts: &[Pt], d: Pt) -> Option<(Pt, Vec<SnapGuide>, Pt)> {
        ...
        if !cfg.enabled || base_pts.is_empty() || (!cfg.key_points && !cfg.object_geometry && !cfg.segment_mids) {
            return None;
        }
        let tol = cfg.radius_px / self.ppu.max(1e-4);
    ...
    fn nearest_edge(&self, pi: usize, pos: Pt) -> Option<(Pt, f32)> {
        ...
        for i in 0..segs {
            ...
            if self.selected.contains(&a.id) || self.selected.contains(&b.id) {
                continue;
            }
    ...
    pub fn snap_anchor(&self, base_pts: &[Pt], d: Pt) -> (Pt, Vec<SnapGuide>, Option<(Pt, String)>) {
        ...
        if !cfg.enabled || base_pts.is_empty() {
            return (d, vec![], hud(add(first, d)));
        }
        let tol = cfg.radius_px / self.ppu.max(1e-4);
        ...
        if cfg.artboard {
            if let Some(ab) = self.doc.active_artboard() {
                let (x0, y0, x1, y1) = ab.rect();
                for c in [[x0, y0], [x1, y0], [x1, y1], [x0, y1], [(x0 + x1) * 0.5, (y0 + y1) * 0.5]] {
                    pts.push(c);
                }
            }
        }
        ...
        if let Some((_, t)) = best {
            ...
            if let Some(pid) = best_pid {
                g.push(SnapGuide::PathHi { pid });
            } // light up the whole snapped path
        ...
        if let Some((diff, tp)) = bx {
            nd[0] += diff;
            sx = true;
            if cfg.alignment_guides {
                guides.push(SnapGuide::Line { a: [tp[0], tp[1].min(moved[1])], b: [tp[0], tp[1].max(moved[1])] });
            }
        }
        ...
        if let Some((diff, tp)) = by {
            nd[1] += diff;
            sy = true;
            if cfg.alignment_guides {
                guides.push(SnapGuide::Line { a: [tp[0].min(moved[0]), tp[1]], b: [tp[0].max(moved[0]), tp[1]] });
            }
        }
        ...
        if cfg.grid {
            ...
            if !sy {
                nd[1] += (moved[1] / step).round() * step - moved[1];
            }
        }
    ...
    pub fn add_anchor(&mut self, pi: usize, i: usize, t: f32) -> u32 {
        ...
        if a.hout.is_none() && b.hin.is_none() {
            ...
            self.doc.paths[pi]
                .anchors
                .insert(i + 1, Anchor { id: nid, p: mid, hin: Some(r0), hout: Some(r1), smooth: true });
        }
        nid
    }
    ...
    pub fn join(&mut self, act: u32, other: u32, end_aid: u32, pos: Pt) {
        ...
        for a in moved {
            self.doc.paths[ai].anchors.push(a);
        }
        self.selected.clear();
        for a in &self.doc.paths[ai].anchors {
            self.selected.insert(a.id);
        }
        ...
        if let Some(aid) = self.nearest_anchor(pos, ANCHOR_R, true) {
            if let Some((pi, ai)) = self.doc.aidx(aid) {
                ...
                let tip = self
                    ...
                    .and_then(|i| self.doc.paths[i].anchors.last().map(|a| a.id));
                if is_end {
    ...
    pub fn pointer_down(&mut self, pos: Pt) {
        ...
        self.gesture_delta = [0.0, 0.0];
        self.gesture_tf = None;
    ...
    pub fn pointer_up(&mut self) {
        ...
        if let Drag::Shape { pid, .. } = self.drag {
            if let Some(pi) = self.doc.pidx(pid) {
                let b = self.doc.bbox(pi);
                if (b.2 - b.0) < 2.0 && (b.3 - b.1) < 2.0 {
                    self.doc.paths.remove(pi);
                    self.dirty = false;
                }
            }
        }
        if let Drag::PenClose { .. } = self.drag {
            self.active = None;
        }
        if matches!(self.drag, Drag::Object { .. }) && (self.gesture_delta[0] != 0.0 || self.gesture_delta[1] != 0.0) {
            self.last_tf = Some((TfAgain::Move(self.gesture_delta), self.gesture_copy));
        ...
        match std::mem::replace(&mut self.drag, Drag::None) {
            ...
            Drag::Segment { pid, i, down, a_out0, b_in0, ap0, bp0, straight } => {
                ...
                if let Some(pi) = self.doc.pidx(pid) {
                    ...
                    let bi = (i + 1) % n;
                    if straight {
                        self.doc.paths[pi].anchors[i].p = add(ap0, d);
                        self.doc.paths[pi].anchors[bi].p = add(bp0, d);
                    } else {
                        self.doc.paths[pi].anchors[i].hout = Some(add(a_out0.unwrap_or(ap0), d));
                        self.doc.paths[pi].anchors[bi].hin = Some(add(b_in0.unwrap_or(bp0), d));
                    }
                }
            ...
            Drag::Guide { idx } => {
                if let Some(g) = self.doc.guides.get(idx).copied() {
                    ...
                    if let Some(gm) = self.doc.guides.get_mut(idx) {
                        gm.pos = v;
                    }
                    let o = self.doc.ruler_origin;
                    self.snap_hud = Some((
                        pos,
                        if g.vertical { format!("X {:.0}", v - o[0]) } else { format!("Y {:.0}", v - o[1]) },
                    ));
                }
                self.drag = Drag::Guide { idx };
                self.dirty = true;
            ...
            Drag::Marquee { start, base } => {
                ...
                for p in &self.doc.paths {
                    ...
                    for (ring, closed) in
                        std::iter::once((&p.anchors, p.closed)).chain(p.holes.iter().map(|h| (h, true)))
                    {
                        let n = ring.len();
                        if n < 2 {
                            continue;
            ...
            Drag::DupPending { srcs, down, object } => {
                if dist(pos, down) < DRAG_THRESH {
                    self.drag = Drag::DupPending { srcs, down, object };
                } else {
            ...
            Drag::Scale { handle, angle, opp_l, cen_l, h0_l, base } => {
                ...
                for (_, p0, _, _) in &base {
                    let q = rotate_about(*p0, [0.0, 0.0], -angle);
                    lx0 = lx0.min(q[0]);
                    ly0 = ly0.min(q[1]);
                    lx1 = lx1.max(q[0]);
                    ly1 = ly1.max(q[1]);
    ...
    pub fn nudge(&mut self, dx: f32, dy: f32) {
        ...
        for aid in ids {
            if let Some(a) = self.doc.anchor_mut(aid) {
                a.p = add(a.p, [dx, dy]);
                a.hin = a.hin.map(|h| add(h, [dx, dy]));
                a.hout = a.hout.map(|h| add(h, [dx, dy]));
            }
        }
        self.dirty = true;
        self.commit();
    ...
    fn selected_pids(&self) -> HashSet<u32> {
        ...
        if let Some(pid) = self.dsel_path {
            pids.insert(pid);
        }
        pids
    }
    ...
    pub fn bump_stroke(&mut self, delta: f32) {
        ...
        for q in pids {
            if let Some(pi) = self.doc.pidx(q) {
                self.doc.paths[pi].stroke_width = w;
            }
        }
        self.dirty = true;
        self.commit();
    ...
    pub fn rename_path(&mut self, pid: u32, name: String) {
        ...
        let n = name.trim();
        if let Some(pi) = self.doc.pidx(pid) {
            self.doc.paths[pi].name = if n.is_empty() { None } else { Some(n.to_string()) };
        }
        self.dirty = true;
        self.commit();
    }
    ...
    pub fn layer_rename(&mut self, nid: u32, name: String) {
        self.begin();
        self.doc.set_node_name(nid, name.trim().to_string());
        self.dirty = true;
        self.commit();
    }
