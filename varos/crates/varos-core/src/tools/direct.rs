...
impl Tool for Direct {
    fn down(&self, ed: &mut Editor, pos: Pt) {
        ...
        if let Some(aid) = ed.handle_hit(pos) {
            ...
            if ed.mods.alt { if let Some(a) = ed.doc.anchor_mut(aid) { a.smooth = false; } }
        ...
        if ed.mods.alt {
            if let Some(aid) = ed.nearest_anchor(pos, ANCHOR_R, false) {
                let (pi, _) = ed.doc.aidx(aid).unwrap(); let pid = ed.doc.paths[pi].id;
                if !ed.mods.shift { ed.selected.clear(); } ed.selected.insert(aid);
                ed.drag = Drag::DupPending { srcs: vec![pid], down: pos, object: false }; return;
            }
            if let Some(pid) = ed.path_under(pos) { ed.drag = Drag::DupPending { srcs: vec![pid], down: pos, object: false }; return; }
        }
        ...
        if let Some(aid) = ed.nearest_anchor(pos, ANCHOR_R, false) {
            ed.dsel_path = None;
            if ed.mods.shift { if ed.selected.contains(&aid) { ed.selected.remove(&aid); } else { ed.selected.insert(aid); } }
            else if !ed.selected.contains(&aid) { ed.selected.clear(); ed.selected.insert(aid); }
            ed.begin_anchor_drag(pos); return;
        }
        ...
        if let Some(pid) = ed.path_under(pos) {
            if let Some(pi) = ed.doc.pidx(pid) {
                if let Some((i, _, d)) = ed.doc.nearest_seg(pi, pos) { if d <= EDGE_R / ed.ppu { ed.start_segment(pid, i, pos); return; } }
            }
            ed.selected.clear();
            ...
            let items = if let Some(pi) = ed.doc.pidx(pid) {
                ed.doc.paths[pi].anchors.iter().chain(ed.doc.paths[pi].holes.iter().flatten()).map(|a| (a.id, a.p, a.hin, a.hout)).collect()
            } else { vec![] };
            ed.drag = Drag::Anchors { start: pos, items };
            return;
        }
        // empty -> marquee
        if !ed.mods.shift { ed.selected.clear(); }
        ed.dsel_path = None;
