    ...
    fn down(&self, ed: &mut Editor, pos: Pt) {
        ...
        if let Some(pid) = ed.path_under(pos) {
            ...
            if ed.mods.alt {
                ...
                let srcs: Vec<u32> = if in_sel && ed.objsel.len() > 1 { ed.objsel.iter().copied().collect() } else { members };
            ...
            if ed.mods.shift {
                ...
                if all { for m in &members { ed.objsel.remove(m); } } else { for m in members { ed.objsel.insert(m); } }
                ed.obj_angle = 0.0;                                          // selection set changed → axis-align
            } else if !members.iter().any(|m| ed.objsel.contains(&m)) {
                ed.objsel.clear(); for m in members { ed.objsel.insert(m); } ed.obj_angle = 0.0; // fresh selection
